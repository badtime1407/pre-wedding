...
// ── สถิติจำนวนการจองในเดือนที่แสดงอยู่ ──
const currentMonthStr = computed(() =>
  `${year.value}-${String(month.value + 1).padStart(2, '0')}`
)

const monthBookings = computed(() =>
  bookings.value.filter(b => b.date?.startsWith(currentMonthStr.value))
)

const stats = computed(() => ({
  total:       monthBookings.value.filter(b => b.status !== "cancelled").length,
  pending:     monthBookings.value.filter(b => b.status === "pending").length,
  in_progress: monthBookings.value.filter(b => b.status === "in_progress").length,
  completed:   monthBookings.value.filter(b => b.status === "completed").length,
  cancelled:   monthBookings.value.filter(b => b.status === "cancelled").length,
}))
...
function getBookingsForDate(date) {
  return bookings.value.filter(b => b.date === date && b.status !== "cancelled")
}
  ...
  <div class="min-h-screen bg-[#f6eee1] flex">
    ...
    <div class="flex-1 p-8 overflow-auto">
      ...
      <div class="max-w-5xl mx-auto mb-6">
        <p class="text-sm text-[#9e8e80]">การจองเดือน{{ thaiMonthName }} ทั้งหมด <span class="font-bold text-[#2c2218]">{{ stats.total }}</span> รายการ</p>
      </div>
        ...
        <div class="max-w-5xl mx-auto rounded-2xl shadow-lg overflow-hidden">
          ...
          <div class="text-black px-8 py-5 flex justify-between items-center">
            <button @click="prevMonth"
              class="w-9 h-9 rounded-full hover:bg-black/10 transition flex items-center justify-center">
              <img src="/left-arrow.png" alt="Previous" class="w-5 h-5"/>
            </button>
            <span class="text-xl font-semibold tracking-wide">{{ thaiMonthName }} {{ thaiYear }}</span>
          ...
          <div class="grid grid-cols-7 bg-[#f5f0e8]">
            <div v-for="d in ['อา','จ','อ','พ','พฤ','ศ','ส']" :key="d"
              class="py-3 text-center text-sm font-semibold text-[#7a6a5a]">{{ d }}</div>
          </div>
          ...
          <div class="grid grid-cols-7 border-t border-[#e8e0d4]">
            ...
            <div v-for="day in daysInMonth" :key="day"
              ...
              :class="[
                getBookingsForDate(fullDate(day)).length > 0 ? 'cursor-pointer hover:bg-[#fdf6ec]' : 'bg-white',
                fullDate(day) === todayStr ? 'bg-amber-50' : ''
              ]"
              @click="openDay(day)">
              <div class="flex justify-between items-start mb-1">
                <span class="text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full"
                  :class="fullDate(day) === todayStr ? 'bg-[#3d2b1f] text-white' : 'text-[#3d2b1f]'">
                  {{ day }}
                </span>
                <span v-if="getBookingsForDate(fullDate(day)).length > 0"
                  class="text-xs bg-[#e7dcc7] text-[#3d2b1f] rounded-full px-1.5 py-0.5 font-medium">
                  {{ getBookingsForDate(fullDate(day)).length }}
                </span>
              </div>
              <div class="space-y-1">
                <div v-for="(booking, i) in getBookingsForDate(fullDate(day)).slice(0, 3)" :key="i"
                  class="text-xs px-2 py-0.5 rounded-md truncate bg-[#e7dcc7] text-[#3d2b1f]">
                  {{ booking.customer_name }}
                </div>
                <div v-if="getBookingsForDate(fullDate(day)).length > 3"
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div class="bg-[#3d2b1f] text-white px-6 py-4 flex justify-between items-center">
          ...
          <button @click="closeModal"
            class="w-8 h-8 rounded-full hover:bg-white/20 transition flex items-center justify-center">✕</button>
        </div>
        ...
        <div class="overflow-y-auto p-6 space-y-4">
          <div v-for="booking in selectedDayBookings" :key="booking.id"
            ...
            <div class="flex items-center gap-2 text-sm text-[#7a6a5a]">
              ...
              <span v-if="booking.package_name"
                class="text-xs bg-[#e7dcc7] text-[#3d2f22] px-2 py-0.5 rounded-full">
                {{ booking.package_name }}
              </span>
              <span v-else class="text-xs text-[#c5b9ac]">ยังไม่มีแพ็คเกจ</span>
            </div>
